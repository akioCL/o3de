...
namespace AssetProcessor
{
    ...
    void AssetProcessorManager::OnJobStatusChanged(JobEntry jobEntry, JobStatus status)
    {
        ...
        AZ::Uuid legacySourceUUID = AssetUtilities::CreateSafeSourceUUIDFromName(jobEntry.m_sourceAssetReference.RelativePath().c_str(), false); // legacy source uuid
    ...
    void AssetProcessorManager::CheckMissingFiles()
    {
        ...
        for (auto iter = m_sourceFilesInDatabase.begin(); iter != m_sourceFilesInDatabase.end(); iter++)
        {
            ...
            CheckDeletedSourceFile(iter.value().m_sourceAssetReference, AZStd::chrono::system_clock::now());
        }
    ...
    bool AssetProcessorManager::CheckForIntermediateAssetLoop(const SourceAssetReference& sourceAsset, AZStd::string_view productAsset)
    {
        ...
        auto productItr = AZStd::find_if(
            ...
            {
                return AZ::StringFunc::Equal(intermediateAsset.RelativePath().Native(), productAsset);
            });
        ...
        for (auto itProcessedAsset = m_assetProcessedList.begin(); itProcessedAsset != m_assetProcessedList.end(); )
        {
            ...
            for (const AssetBuilderSDK::JobProduct& product : itProcessedAsset->m_response.m_outputProducts)
            {
                ...
                if (productWrapper.HasIntermediateProduct() && CheckForIntermediateAssetLoop(
                        itProcessedAsset->m_entry.m_sourceAssetReference, productPath.GetRelativePath()))
                {
                    // Loop detected
                ...
                if (auto result = CheckIntermediateProductConflict(productWrapper.HasIntermediateProduct(),
                    productWrapper.HasIntermediateProduct() ? productPath.GetRelativePath().c_str() : itProcessedAsset->m_entry.m_sourceAssetReference.RelativePath().c_str());
        ...
        AZStd::chrono::system_clock::time_point initialProcessTime)
    {
        ...
        m_stateData->GetSourceFileDependenciesByDependsOnSource(sourceAsset.RelativePath().c_str(), SourceFileDependencyEntry::DEP_Any, results);
        ...
        for (SourceFileDependencyEntry& existingEntry : results)
        {
            ...
            existingEntry.m_dependsOnSource = PlaceHolderFileName + sourceAsset.RelativePath().Native();
    ...
    void AssetProcessorManager::ProcessJob(JobDetails& job)
    {
        ...
        QueryAbsolutePathDependenciesRecursive(job.m_jobEntry.m_sourceAssetReference.RelativePath().c_str(), job.m_fingerprintFiles, AzToolsFramework::AssetDatabase::SourceFileDependencyEntry::DEP_SourceToSource, false);
    ...
    void AssetProcessorManager::ProcessBuilders(QString normalizedPath, QString databasePathToFile, const ScanFolderInfo* scanFolder, const AssetProcessor::BuilderInfoList& builderInfoList)
    {
...

        {
            ...
            m_sourceUUIDToSourceInfoMap.insert(AZStd::make_pair(sourceUUID, sourceAsset));
        }
    ...
    bool AssetProcessorManager::ResolveSourceFileDependencyPath(const AssetBuilderSDK::SourceFileDependency& sourceDependency, QString& resultDatabaseSourceName, QStringList& resolvedDependencyList)
    {
        ...
        if (!sourceDependency.m_sourceFileDependencyUUID.IsNull())
        {
            ...
            SourceAssetReference resultSourceInfo;
            ...
            {
                resultDatabaseSourceName = resultSourceInfo.RelativePath().c_str();
            }
        }
    ...
    void AssetProcessorManager::UpdateSourceFileDependenciesDatabase(JobToProcessEntry& entry)
    {
            ...
            for (const AssetProcessor::JobDependencyInternal& jobDependency : jobToCheck.m_jobDependencyList)
            {
                ...
                for (const auto& thisEntry : resolvedDependencyList)
                {
                    SourceFileDependencyEntry newDependencyEntry(
                        builderId, entry.m_sourceFileInfo.m_sourceAssetReference.RelativePath().c_str(), thisEntry.toUtf8().constData(),
                        JobDependencyType,
                        false,
                        subIds.c_str());
                ...
                {
                    SourceFileDependencyEntry newDependencyEntry(
                        builderId, entry.m_sourceFileInfo.m_sourceAssetReference.RelativePath().c_str(), resolvedDatabaseName.toUtf8().constData(),
        ...
        m_stateData->QueryDependsOnSourceBySourceDependency(
            entry.m_sourceFileInfo.m_sourceAssetReference.RelativePath().c_str(), // find all rows in the database where this is the source column
            nullptr, // no filter
        ...
        QString ourNameWithPlaceholder = QString(PlaceHolderFileName) + entry.m_sourceFileInfo.m_sourceAssetReference.RelativePath().c_str();
        ...
        AZStd::string databaseNameEncoded = entry.m_sourceFileInfo.m_sourceAssetReference.RelativePath().c_str();
