    ...
    namespace SceneAPI
    {
        namespace Containers
        {
            ...
            bool SceneManifest::IsEmpty() const
            {
                ...
                AZ_Assert(
                    m_values.empty() == m_storageLookup.empty(),
                    "SceneManifest values and storage-lookup tables have gone out of lockstep.");
            ...
            size_t SceneManifest::GetEntryCount() const
            {
                ...
                AZ_Assert(
                    m_values.size() == m_storageLookup.size(),
                    "SceneManifest values and storage-lookup tables have gone out of lockstep. (%i vs. %i)", m_values.size(),
                    m_storageLookup.size());
