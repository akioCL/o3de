...
namespace AZ
{
    namespace Render
    {
        ...
        void FocusedEntity::Draw()
        {
            ...
            if (const auto modelLodIndex = GetModelLodIndex(drawabaleMetaData.GetView(), drawabaleMetaData.GetFeatureProcessor());
                m_modelLodIndex != modelLodIndex)
            {
                CreateOrUpdateMeshDrawPackets(drawabaleMetaData.GetFeatureProcessor(), modelLodIndex);
            }
        ...
        RPI::ModelLodIndex FocusedEntity::GetModelLodIndex(
            const RPI::ViewPtr view, const MeshFeatureProcessorInterface* featureProcessor) const
        {
            const auto worldTM = GetWorldTransformForEntity(m_entityId);
            const auto modelAsset = featureProcessor->GetModelAsset(*m_meshHandle);
        ...
        void FocusedEntity::OnAcquireMesh(const MeshFeatureProcessorInterface::MeshHandle* meshHandle)
        {
            ...
            const auto modelLodIndex = GetModelLodIndex(view, featureProcessor);
            CreateOrUpdateMeshDrawPackets(featureProcessor, modelLodIndex);
        }
        
        void FocusedEntity::OnMeshDataUpdate()
        {
            const auto scene = RPI::Scene::GetSceneForEntityId(m_entityId);
            const auto view = GetViewFromScene(scene);
            const auto* featureProcessor = scene->GetFeatureProcessor<MeshFeatureProcessorInterface>();
        ...
        void FocusedEntity::CreateOrUpdateMeshDrawPackets(
            const MeshFeatureProcessorInterface* featureProcessor, const RPI::ModelLodIndex
        modelLodIndex)
        {
            ...
            if (m_meshHandle->IsValid())
            {
                ...
                m_meshDrawPackets = BuildMeshDrawPackets(
                    m_modelLodIndex,
                    featureProcessor->GetModelAsset(*m_meshHandle),
                    m_maskMaterial,
                    maskMeshObjectSrg);
            }
        }
