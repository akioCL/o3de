...
namespace AZ
{
    namespace Render
    {
        class FocusedEntity : private AZ::Render::AtomMeshNotificationBus::Handler
        {
        public:
        ...
        private:
            RPI::ModelLodIndex GetModelLodIndex(const RPI::ViewPtr view, const MeshFeatureProcessorInterface* featureProcessor) const;

            void OnAcquireMesh(const MeshFeatureProcessorInterface::MeshHandle* meshHandle) override;

            void OnMeshDataUpdate() override;

            void CreateOrUpdateMeshDrawPackets(
                const MeshFeatureProcessorInterface* featureProcessor, const RPI::ModelLodIndex modelLodIndex);
