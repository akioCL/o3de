...
namespace RenderJoy
{
    ...
    void RenderJoyTrianglePass::OnShaderVariantReinitialized(const AZ::RPI::Shader&, const AZ::RPI::ShaderVariantId&, AZ::RPI::ShaderVariantStableId)
    {
        LoadShader();
    }
    ///////////////////////////////////////////////////////////////////
    ...
    void RenderJoyTrianglePass::LoadShader()
    {
        ...
        // Load Pass SRG
        const AZ::Data::Asset<AZ::RPI::ShaderResourceGroupAsset>& passSrgAsset = m_shader->FindShaderResourceGroupAsset(AZ::Name{ "PassSrg" });
        if (passSrgAsset)
        {
            m_shaderResourceGroup = AZ::RPI::ShaderResourceGroup::Create(passSrgAsset);
    ...
    void RenderJoyTrianglePass::BuildAttachmentsInternal()
    {
    ...
    void RenderJoyTrianglePass::CreateImageForChannel(uint32_t imageChannelIdx, const AZStd::string& imageName, size_t width, size_t height, uint32_t color)
    {
        ...
        const uint32_t pixelDataSize = width * height * sizeof(color);
